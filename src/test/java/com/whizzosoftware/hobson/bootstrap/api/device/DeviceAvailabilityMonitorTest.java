...
public class DeviceAvailabilityMonitorTest {
    ...
    public void testRun() {
        ...
        assertEquals("local:local:plugin:device1", em.getEvent(0).getProperties().get(DeviceUnavailableEvent.PROP_DEVICE_CONTEXT).toString());
        assertTrue(em.getEvent(1) instanceof DeviceUnavailableEvent);
        assertEquals("local:local:plugin:device2", em.getEvent(1).getProperties().get(DeviceUnavailableEvent.PROP_DEVICE_CONTEXT).toString());
        em.clearEvents();
